    ...
    def pip_install(self, part_dir, build_dir, src_dir, extra_args):
        ...
        if sys.platform in ('darwin',):
            ...
            if major_version > 7:
                ...
                if xcode_path.startswith('/Applications'):
                    isys_glob = xcode_path + "/Platforms/MacOSX.platform/Developer/SDKs/MacOSX10*"
                else:
                    isys_glob = '/Developer/SDKs/MacOSX10*'

                isysroot = glob.glob(isys_glob)[-1]
                env.update({
